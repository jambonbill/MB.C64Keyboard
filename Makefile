...
CC=sdcc
CFLAGS=--verbose -S -mpic16 -p18F452 --fstack --fommit-frame-pointer --optimize-goto --optimize-cmp --disable-warning 85 --obanksel=2 -pleave-reset-vector
...
OBJS=$(OUTDIR)/pic18f452.o $(OUTDIR)/sm_simple.o $(OUTDIR)/main.o
...
$(OUTDIR)/pic18f452.o: $(OUTDIR)/pic18f452.asm
	$(GPASM) $< -o $@
$(OUTDIR)/sm_simple.o: sm_simple.asm
	$(GPASM) $< -o $@
